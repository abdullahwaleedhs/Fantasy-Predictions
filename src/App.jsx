...
function computeLeagueRanking(league, matches, allPredictionRows) {
  ...
  for (const row of allPredictionRows) {
    ...
    if (!byUser[row.user_id]) byUser[row.user_id] = { points: 0, tierCounts: { 10: 0, 5: 0, 4: 0, 3: 0, 1: 0, 0: 0, none: 0 }, firstPredAt: null };
    const entry = byUser[row.user_id];
    // Earliest prediction the user entered — used to break ties (predicted first = higher rank).
  ...
  return [...league.players]
    .map((p) => {
      ...
      return {
        ...
        firstPredAt: s ? s.firstPredAt : null,
      };
    })
    .sort((a, b) => {
      if (b.points !== a.points) return b.points - a.points;
  ...
  const buildList = (filterFn) =>
    allPredictionRows
      ...
      .map((r) => {
        ...
        let pts = null;
        if (m && finished) {
          const res = matchPointsForRow(r, m);
        ...
        return {
          ...
          pts,
          finished,
          extra: r.double_pred ? `${r.pred_home ?? "—"}-${r.pred_away ?? "—"} / ${r.pred_home_2 ?? "—"}-${r.pred_away_2 ?? "—"}` : null,
        };
      })
      .sort((a, b) => new Date(b.when || 0) - new Date(a.when || 0));

  const boosts = buildList((r) => r.user_boost);
  const doubles = buildList((r) => r.double_pred);

  const Section = ({ title, color, items, emptyText }) => (
    <div style={{ marginBottom: "24px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "12px" }}>
        <Zap size={16} color={color} />
        <div style={{ fontSize: "13px", fontWeight: 800, color: theme.text }}>{title} ({items.length})</div>
      </div>
      {items.length === 0 ? (
        <div style={{ textAlign: "center", color: theme.muted, fontSize: "13px", padding: "20px 0" }}>{emptyText}</div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          {items.map((b) => (
            <div key={b.key} style={{ background: theme.surface, border: `1.5px solid ${color}`, borderRadius: "12px", padding: "12px 14px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
                ...
                <div style={{ marginInlineStart: "auto", fontSize: "13px", fontWeight: 900, color: b.finished ? color : theme.muted }}>
              ...
              {b.extra && (
                <div style={{ fontSize: "11px", color: theme.muted, marginBottom: "2px" }}>📝 التوقعان: {b.extra}</div>
              )}
              {b.when && (
    ...
    <div style={{ padding: "24px 18px 60px" }}>
      <div style={{ maxWidth: "560px", margin: "0 auto" }}>
        ...
        <p style={{ fontSize: "12px", color: theme.muted, textAlign: "center", marginBottom: "18px" }}>
          سجل استخدام المزايا — من استخدمها، متى، على أي مباراة، وكم جاب فيها
        </p>

        <Section title="التربل ×٣" color="#10B981" items={boosts} emptyText="ما استخدم أحد التربل بعد." />
        <Section title="توقعين" color="#3B82F6" items={doubles} emptyText="ما استخدم أحد توقعين بعد." />
      </div>
    </div>
  );
}
...
function PrivateLeagueDetail({ league, matches, allPredictionRows, onJoin, onSetCountMode, onBack, tournaments, tournamentLogos, currentUser, onViewProfile, theme }) {
  ...
  for (const row of allPredictionRows) {
    ...
    if (!realPointsByUserId[row.user_id]) {
      realPointsByUserId[row.user_id] = { points: 0, tierCounts: { 10: 0, 5: 0, 4: 0, 3: 0, 1: 0, 0: 0, none: 0 }, firstPredAt: null };
    }
    const entry = realPointsByUserId[row.user_id];

    // Earliest prediction — ties are broken by who predicted first.
    if (row.updated_at && (entry.firstPredAt == null || new Date(row.updated_at) < new Date(entry.firstPredAt))) {
      entry.firstPredAt = row.updated_at;
    }
  ...
  const ranked = [...league.players]
    .map((p) => {
      ...
      return {
        ...
        firstPredAt: stats ? stats.firstPredAt : null,
      };
    })
    .sort((a, b) => {
      if (b.points !== a.points) return b.points - a.points;
